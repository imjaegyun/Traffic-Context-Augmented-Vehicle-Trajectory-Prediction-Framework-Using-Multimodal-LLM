...
class Generator(nn.Module):
    def __init__(self, seq_len, out_len, feature_size=2, hidden_dim=128):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(seq_len * feature_size, hidden_dim),
            nn.ReLU(),
        ...
        self.decoder = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, out_len * feature_size)
        )

    def forward(self, x, noise_scale=0.0):
        B, C, T = x.size()  # (B, 2, T_in)
        x = x.permute(0, 2, 1).reshape(B, -1)  # (B, T_in * 2)
        h = self.encoder(x)  # (B, hidden_dim)
        out = self.decoder(h)  # (B, T_out * 2)
        out = out.view(B, self.out_len, self.feature_size)  # (B, T_out, 2)
        if noise_scale > 0:
...
class Discriminator(nn.Module):
    def __init__(self, seq_len, out_len, feature_size=2, hidden_dim=128):
        ...
        self.net = nn.Sequential(
            nn.Linear((seq_len + out_len) * feature_size, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(),
    ...
    def forward(self, x, y):
        B = x.size(0)
        x_flat = x.permute(0, 2, 1).reshape(B, -1)  # (B, T_in * 2)
        y_flat = y.permute(0, 2, 1).reshape(B, -1)  # (B, T_out * 2)
        combined = torch.cat([x_flat, y_flat], dim=1)  # (B, (T_in + T_out) * 2)
        return self.net(combined)  # (B, 1)

# 시각화 함수
def visualize_one_sample(generator, sample, device, idx=0, prefix="test_sample", save_dir="vis_gail"):
    ...
    with torch.no_grad():
        pred = generator(x_in, noise_scale=0.0)  # 시각화 시 노이즈 없음
    ...
    plt.title(f"{prefix} idx={idx} (GAIL)")
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.grid(True)
    plt.legend()
...
def train_ddp(args):
    ...
    # GAIL 모델 초기화
    generator = Generator(seq_len=args["seq_len"], out_len=args["out_len"], feature_size=2, hidden_dim=128).to(device)
    discriminator = Discriminator(seq_len=args["seq_len"], out_len=args["out_len"], feature_size=2, hidden_dim=128).to(device)

    ddp_generator = pnl.DistributedDataParallel(generator, device_ids=[local_rank], output_device=local_rank)
    ddp_discriminator = pnl.DistributedDataParallel(discriminator, device_ids=[local_rank], output_device=local_rank)

    g_optimizer = torch.optim.AdamW(ddp_generator.parameters(), lr=args["lr"], weight_decay=1e-4)
    d_optimizer = torch.optim.AdamW(ddp_discriminator.parameters(), lr=args["lr"], weight_decay=1e-4)
    ...
    best_ckpt_path = "gail_best_val_checkpoint.pt"
    ...
    #         y = batch_data["target_traj"].permute(0, 2, 1).to(device)  # (B, T_out, 2)
    #         y = y.permute(0, 2, 1)  # (B, 2, T_out)

    #         # 1. Discriminator 학습
    #         d_optimizer.zero_grad()
    #         real_traj = torch.cat([x, y], dim=2)  # (B, 2, T_in + T_out)
    #         real_labels = torch.ones(x.size(0), 1, device=device)
    #         d_real = ddp_discriminator(x, y)
    #         d_real_loss = criterion(d_real, real_labels)

    #         gen_traj = ddp_generator(x)  # (B, T_out, 2)
    #         gen_traj = gen_traj.permute(0, 2, 1)  # (B, 2, T_out)
    #         fake_labels = torch.zeros(x.size(0), 1, device=device)
    #         d_fake = ddp_discriminator(x, gen_traj)
    ...
    #         gen_traj = ddp_generator(x)  # (B, T_out, 2)
    #         gen_traj_for_d = gen_traj.permute(0, 2, 1)  # (B, 2, T_out)
    #         d_output = ddp_discriminator(x, gen_traj_for_d)
    #         g_loss = criterion(d_output, real_labels)  # Generator는 Discriminator를 속이려 함
    #         g_loss.backward()
    #         g_optimizer.step()
    ...
    #             y_v = y_v.permute(0, 2, 1)  # (B, 2, T_out)
    #             gen_traj_v = ddp_generator(x_v)
    #             gen_traj_v_for_d = gen_traj_v.permute(0, 2, 1)  # (B, 2, T_out)
    #             d_output_v = ddp_discriminator(x_v, gen_traj_v_for_d)
    #             g_loss_v = criterion(d_output_v, torch.ones(x_v.size(0), 1, device=device))
    #             val_g_loss += g_loss_v.item()
    #     avg_val_g_loss = val_g_loss / len(val_loader)

    #     if local_rank == 0:
    #         print(f"[Epoch {epoch+1}/{epochs}] G Loss={avg_g_loss:.4f}, D Loss={avg_d_loss:.4f}, Val G Loss={avg_val_g_loss:.4f} (GAIL)")
    #         if avg_val_g_loss < best_val_loss:
    #             best_val_loss = avg_val_g_loss
    #             torch.save(ddp_generator.module.state_dict(), best_ckpt_path)
    ...
    #         visualize_one_sample(ddp_generator.module, sample_val, device, idx=epoch,
    #                              prefix="val_sample", save_dir="vis_gail_val")

    # 학습된 모델 로드
    if local_rank == 0 and os.path.exists(best_ckpt_path):
        checkpoint = torch.load(best_ckpt_path, map_location=device)
        ddp_generator.module.load_state_dict(checkpoint)
        print(f"Loaded generator from {best_ckpt_path} for testing")
    else:
        if local_rank == 0:
            print(f"Warning: Checkpoint {best_ckpt_path} not found. Testing with untrained generator.")
        ...
        with torch.no_grad():
            for batch_data in test_loader:
                ...
                for _ in range(num_candidates):
                    pred = ddp_generator(x_t, noise_scale=0.05)  # 노이즈 추가로 다양성 부여
            ...
            print(f"[Test] minADE={avg_min_ade:.4f}, minFDE={avg_min_fde:.4f}, minRMSE={avg_min_rmse:.4f} (GAIL)")

            # 시각화 (첫 번째 후보 사용)
            for i in range(min(5, len(test_dataset))):
                sample = test_dataset[i]
                visualize_one_sample(ddp_generator.module, sample, device, idx=i,
                                    prefix="test_sample", save_dir="vis_gail_test")

...
class SocialSTGCNN(nn.Module):
    def __init__(self, seq_len, out_len, feature_size=2, hidden_dim=128, kernel_size=3):
        super(SocialSTGCNN, self).__init__()
        self.seq_len = seq_len
        self.out_len = out_len
        self.feature_size = feature_size
        self.input_embedding = nn.Linear(feature_size, hidden_dim)
        self.stgconv1 = STGConv(hidden_dim, hidden_dim, kernel_size)
        self.stgconv2 = STGConv(hidden_dim, hidden_dim, kernel_size)
        self.output_layer = nn.Linear(hidden_dim * seq_len, out_len * feature_size)
    ...
    def forward(self, x, y=None, norm_stat=None):
        ...
        pred = pred.view(B, self.out_len, self.feature_size).permute(0, 2, 1)  # (B, 2, T_out)
...
def visualize_one_sample_with_candidates(model, sample, device, num_candidates=5, idx=0,
                                        prefix="test_sample", save_dir="vis_social_stgcnn_test"):
...
def train_ddp(args):
    ...
    # 모델 초기화
    model = SocialSTGCNN(seq_len=args["seq_len"], out_len=args["out_len"], feature_size=2,
                         hidden_dim=128, kernel_size=3).to(device)
    ...
    if local_rank == 0:
        ...
        with torch.no_grad():
            ddp_model.train()  # 다중 후보 생성을 위해 train 모드 (Dropout 등 활성화)
            ...
            print(f"[Test] minADE={avg_min_ade:.4f}, minFDE={avg_min_fde:.4f}, minRMSE={avg_min_rmse:.4f} (Social-STGCNN)")

            # 시각화: 테스트 세트 중 임의 샘플 선택
            if len(test_dataset) > 0:
                ...
                visualize_one_sample_with_candidates(
                    ...
                    idx=sample_idx,
                    prefix="test_sample",
                    save_dir="vis_social_stgcnn_test"
                )

...
class TrajectronPP(nn.Module):
    ...
    def forward(self, x, y=None, norm_stat=None):
        ...
        z = z.unsqueeze(1).repeat(1, self.out_len, 1)
        last_in = x[:, -1:, :]
        last_in = last_in.repeat(1, self.out_len, 1)
        decoder_input = torch.cat([last_in, z], dim=-1)
...
# 시각화 함수
def visualize_one_sample(model, sample, device, idx=0, prefix="test_sample", save_dir="vis_trajectronpp"):
    ...
    plt.title(f"{prefix} idx={idx} (Trajectron++)")
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.grid(True)
    plt.legend()
...
def train_ddp(args):
    ...
    #     if local_rank == 0:
    #         print(f"[Epoch {epoch+1}/{epochs}] Train Loss={avg_loss:.4f}, Val Loss={avg_val_loss:.4f} (Trajectron++)")
    ...
    #                             prefix="val_sample", save_dir="vis_trajectronpp_val")
        ...
        with torch.no_grad():
            ...
            for batch_data in test_loader:
                ...
                for i in range(num_candidates):
                    out_t = ddp_model(x_t, y=None, norm_stat=None)

...
from graph import lane_roi  # graph.py에서 lane_roi만 가져옴
...
def build_dataset_from_tracks_sliding(track_list,
    ...
    for track_idx, item in enumerate(track_list):
        ...
        for start in range(0, N - (seq_len + out_len) + 1, stride):
            ...
            # 실제 answer는 filtered_ctx 등으로 적절히 구성(예시)
...
########################################
# LlamaMultiModal
# ==> 여기서 labels 길이 = fused_embeds 길이를 맞춰주는 것이 핵심
########################################
class LlamaMultiModal(nn.Module):
    ...
    def forward(self, 
        ...
        image_tokens = self.qformer(vision_embs)  # (B, num_query_tokens, q_hidden)
        image_tokens = self.q_proj(image_tokens)  # -> (B, num_query_tokens, llama_hidden)
        image_tokens = image_tokens + self.vision_modality_embedding

        if (input_ids is not None) and (attention_mask is not None):
            # 이미 Prompt+Answer가 들어있는 경우
            ...
            fused_embeds = torch.cat([image_tokens, text_embeds], dim=1)  # (B, Tv_q + L_text, hidden)

            # mask
            img_mask = torch.ones((B, image_tokens.size(1)), dtype=attention_mask.dtype, device=device)
            fused_mask = torch.cat([img_mask, attention_mask], dim=1)  # (B, Tv_q + L_text)

            # ====> 여기서 labels도 이미지 토큰 길이만큼 -100을 앞에 붙여줘야 
            #      fused_embeds와 동일 길이가 됨.
            ...
            input_ids_ = text_inputs["input_ids"]  # (B, L)
    ...
    def generate_batch(self,
        ...
        for out_ids in outputs:
            ...
            # 후처리: "No right-following vehicle." 이후의 텍스트 제거
...
def visualize_one_sample(model, sample, device):
    ...
    lane_polygon = sample["lane_polygon"].numpy()  # ROI 정보
    lane_polygon_len = sample["lane_polygon_len"]
...
def visualize_test_samples(model, dataset, device, num_samples=5,
    ...
    for idx in indices:
        sample = dataset[idx]
        in_den, gt_den, pd_den = visualize_one_sample(model, sample, device)
        all_in_dens.append(in_den)
        all_gt_dens.append(gt_den)
        all_pd_dens.append(pd_den)
    ...
    # ROI 그리기 (레전드에 포함되지 않음)
    lanes = lane_roi["Site C"]["A"]
    colors = ['purple', 'blue', 'green', 'orange', 'cyan', 'magenta', 'yellow']
    for i, (lane_num, coords) in enumerate(lanes.items()):
        coords_arr = np.array(coords)
        coords_arr = np.vstack([coords_arr, coords_arr[0]])
        ax.plot(coords_arr[:, 0], coords_arr[:, 1], color=colors[i % len(colors)], \
                linestyle='-', alpha=0.3)

    # 궤적 그리기 (첫 번째만 레전드에 포함)
    for j, in_den in enumerate(all_in_dens):
        lbl = "Input Trajectory" if j == 0 else None
        ax.plot(in_den[:, 0], in_den[:, 1], 'b-', linewidth=2, label=lbl)
    for j, gt_den in enumerate(all_gt_dens):
        lbl = "Ground Truth Trajectory" if j == 0 else None
        ax.plot(gt_den[:, 0], gt_den[:, 1], 'g-', linewidth=2, label=lbl)
    for j, pd_den in enumerate(all_pd_dens):
        lbl = "Predicted Trajectory" if j == 0 else None
        ax.plot(pd_den[:, 0], pd_den[:, 1], 'r--', alpha=0.8, linewidth=2, label=lbl)
    ...
    # 단일 파일로 저장
    filename = f"{prefix}_all_trajectories.png"
    save_path = os.path.join(save_dir, filename)
    plt.savefig(save_path, dpi=300, transparent=True)
    plt.close()
    print(f"[{prefix}] All trajectories with legend saved to {save_path}")
...
def train_ddp(args):
    ...
    ddp_model.module.mllm.load_state_dict(state_dict, strict=True)
    ...
    best_ckpt_path= "/home/user/MLLM/models/6_30_best_val_checkpoint.pt"
    ...
    if local_rank==0:
        ...
        with torch.no_grad():
            for batch_data in test_loader:
                ...
                # RMSE 계산: 모든 시간 단계에 대한 제곱 오차의 평균 후 제곱근
        ...
        visualize_test_samples(ddp_model.module, test_dataset, device,
                               num_samples=5, prefix="test_sample", save_dir="plot_test_visualization")
...
def main():
    ...
    torch.cuda.manual_seed_all(42)  # 모든 GPU에 대해 시드 고정
    torch.backends.cudnn.deterministic = True  # 결정적 연산 보장

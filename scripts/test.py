...
class LlamaMultiModal(nn.Module):
    ...
    def generate_batch(self,
        ...
        self.llama_wrapper.llama_model.set_input_embeddings(
            nn.Embedding(orig_embedding.num_embeddings, orig_embedding.embedding_dim).to(device)
        )
        ...
        for out_ids in outputs:
            ...
            # 후처리: "No right-following vehicle." 이후의 텍스트 제거 (예시)
...
###############################################################
# 새롭게 추가된 시각화 함수 (여러 후보 예측을 점선으로 표시)
###############################################################
def visualize_one_sample_with_candidates(model, sample, device,
                                         num_candidates=5,
                                         idx=0,
                                         prefix="test_sample",
                                         save_dir="visualization_test_ddp"):
    """
    test 시에 여러 후보 예측을 얻어, 입력/GT 궤적(실선) + 후보 예측 궤적(점선)을 시각화
    """
    ...
    candidate_preds = []
    with torch.no_grad():
        # 다양성을 위해 num_candidates번 forward
        for _ in range(num_candidates):
            out = model(
                x_in, vision, ctx,
                polygon, poly_len,
                y=None, norm_stat=None,  # 정답/정규화 통계 주지 않음
                input_ids=in_ids,
                attention_mask=attn_m,
                labels=None
            )
            # 모델이 튜플을 반환하면 (loss, decoded) 구조일 수 있으므로 처리
...
def train_ddp(args):
    ...
    best_ckpt_path = "/home/user/MLLM/modify_scripts/3_3_best_val_checkpoint.pt"
    ...
    #             device=device
    #         )
    #         print(f"[Epoch {epoch+1}] Generation sample:\n{gen_texts[0]}\n")
    ...
    if local_rank == 0:
        ...
        with torch.no_grad():
            ...
            for batch_data in test_loader:
                ...
                B_ = x_t.size(0)
                candidate_preds = []  # 각 후보 예측 저장
                
                # num_candidates 번 반복
                for _ in range(num_candidates):
                    out_candidate = ddp_model(
                ...
                # 후보 예측 복원
                # candidate_preds: (B, num_candidates, 2, T_out)
                ...
                # 오차 계산 (ADE, FDE, RMSE)
                # errors: (B, num_candidates, T_out)
                errors = torch.sqrt(((pred_denorm - y_denorm) ** 2).sum(dim=2))  # 2->(x,y)
            ...
            # 시각화: 테스트 세트 중 임의 하나를 선택해 다중 후보 예측을 점선으로 표시
            if len(test_dataset) > 0:
                ...
                visualize_one_sample_with_candidates(
                    ...
                    idx=sample_idx,
                    prefix="test_sample",
                    save_dir="3_3_test_visualization"
                )

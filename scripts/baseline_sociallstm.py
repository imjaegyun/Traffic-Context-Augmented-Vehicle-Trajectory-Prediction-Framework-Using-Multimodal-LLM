...
class SocialLSTM(nn.Module):
    def __init__(self, seq_len, out_len, feature_size=2, embedding_dim=64, hidden_dim=128):
        ...
        # 출력 레이어
        self.fc = nn.Linear(hidden_dim, out_len * feature_size)
    ...
    def forward(self, x, y=None, norm_stat=None):
        ...
        x_embed = self.embedding(x)  # (B, T_in, embedding_dim)

        # LSTM 처리
        lstm_out, _ = self.lstm(x_embed)  # (B, T_in, hidden_dim)
        lstm_out = lstm_out[:, -1, :]  # 마지막 시간 단계: (B, hidden_dim)

        # 출력 생성
        pred = self.fc(lstm_out)  # (B, T_out * 2)
        ...
        if y is not None and norm_stat is not None:
            ...
            pred_den = pred.clone()  # (B, T_out, 2)
            gt_den = y.clone()  # (B, T_out, 2)로 이미 제공됨
...
# 시각화 함수
def visualize_one_sample(model, sample, device, idx=0, prefix="test_sample", save_dir="vis_social_lstm"):
    ...
    plt.title(f"{prefix} idx={idx} (Social-LSTM)")
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.grid(True)
    plt.legend()
...
def train_ddp(args):
    ...
    # Social-LSTM 모델 초기화
    model = SocialLSTM(seq_len=args["seq_len"], out_len=args["out_len"], feature_size=2,
    ...
    best_ckpt_path = "social_lstm_best_val_checkpoint.pt"
    ...
    #             ns_v = batch_data["norm_stat"]
    #             ret_v = ddp_model(x_v, y=y_v, norm_stat=ns_v)
    #             if isinstance(ret_v, tuple):
    #                 lv, _ = ret_v
    #             else:
    ...
    #     if local_rank == 0:
    #         print(f"[Epoch {epoch+1}/{epochs}] Train Loss={avg_loss:.4f}, Val Loss={avg_val_loss:.4f} (Social-LSTM)")
    ...
    #                              prefix="val_sample", save_dir="vis_social_lstm_val")
    ...
    if local_rank == 0:
        ...
        with torch.no_grad():
            ...
            print(f"[Test] minADE={avg_min_ade:.4f}, minFDE={avg_min_fde:.4f}, minRMSE={avg_min_rmse:.4f} (Social-LSTM)")
            ...
            for i in range(min(5, len(test_dataset))):
                ...
                visualize_one_sample(ddp_model.module, sample, device, idx=i,
                                    prefix="test_sample", save_dir="vis_social_lstm_test")

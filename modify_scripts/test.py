...
def check_data_sanity(all_data, max_coord_threshold=1e6):
    ...
    for d in all_data:
        ...
        if np.abs(raw_traj).max() > max_coord_threshold:
            continue
        clean_data.append(d)
    debug_print(f"[check_data_sanity] clean_data: {len(clean_data)} / {len(all_data)}", force=True)
    return clean_data
...
def split_all_data(all_data, train_ratio=0.7, val_ratio=0.2, test_ratio=0.1):
    ...
    debug_print(f"Data split: {len(train_data)} train, {len(val_data)} val, {len(test_data)} test")
...
def parse_lane_from_context(context_str: str):
    ...
    return "safe" if lane_str == "safe" else lane_str[1:]
...
def is_trajectory_abnormal(raw_traj, lane_label=None,
    ...
    if lane_label is not None:
        ...
        if lane_label == "R2L" and np.any(x_vals[1:] > x_vals[:-1]):
            return True
        elif lane_label == "L2R" and np.any(x_vals[1:] < x_vals[:-1]):
            return True
    return False
...
def build_dataset_from_tracks_sliding(track_list,
    ...
    inputs_list = []
    outputs_list = []
    debug_print("Start building dataset from tracks...")
    ...
    debug_print(f"Built {len(inputs_list)} samples from track data")
...
class LlamaWithCrossAttnPEFT(nn.Module):
    def __init__(self, base_model_name, use_lora=True, lora_r=8,
...
class LlamaMultiModal(nn.Module):
    def __init__(self, base_model_name="meta-llama/Llama-2-7b-hf",
                 use_lora=True, lora_r=8, lora_alpha=32, lora_dropout=0.1,
                 vision_dim=512, q_hidden_size=768, q_nhead=8,
                 q_enc_layers=4, q_dec_layers=4, q_num_query_tokens=16):
        super().__init__()
        self.qformer = BlipQFormer(
            vision_dim=vision_dim, hidden_size=q_hidden_size, nhead=q_nhead,
            num_encoder_layers=q_enc_layers, num_decoder_layers=q_dec_layers,
            num_query_tokens=q_num_query_tokens
        )
        self.q_hidden_size = q_hidden_size
        self.llama_wrapper = LlamaWithCrossAttnPEFT(
            base_model_name=base_model_name, use_lora=use_lora,
    ...
    def forward(self, vision_embs, context_str, input_ids=None,
        ...
        if (input_ids is not None) and (attention_mask is not None):
            ...
            if labels is not None:
                fused_labels = torch.full((B, image_tokens.size(1) + labels.size(1)),
                                           -100, dtype=labels.dtype, device=device)
                fused_labels[:, image_tokens.size(1):] = labels
            else:
                fused_labels = None
            outputs = self.llama_wrapper(
                inputs_embeds=fused_embeds, attention_mask=fused_mask,
                labels=fused_labels, output_hidden_states=True
            )
...
class TransformerLTSF(nn.Module):
    def __init__(self, seq_len, out_len, individual, feature_size,
                 ...
                 nhead=1, dropout_rate=0.1, cross_dim=768,
...
class MultiModalTrajectoryModel(nn.Module):
    def __init__(self, seq_len, out_len, individual, feature_size=2,
                 d_model=64, lane_polygon_d_model=64, lane_polygon_nhead=4,
                 lane_polygon_layers=2, max_polygon_points=64, use_post_mlp=True,
                 post_mlp_hidden_dim=64, base_model_name="meta-llama/Llama-7B",
                 use_lora=True, lora_r=8, lora_alpha=32, lora_dropout=0.1,
                 vision_dim=512, q_hidden_size=768, q_nhead=8, q_enc_layers=4,
                 q_dec_layers=4, q_num_query_tokens=16, ltsf_nhead=1,
                 ltsf_dropout=0.1):
        super().__init__()
        self.lane_polygon_encoder = LanePolygonEncoder(
            d_model=lane_polygon_d_model, nhead=lane_polygon_nhead,
            num_layers=lane_polygon_layers, max_points=max_polygon_points
        )
        self.mllm = LlamaMultiModal(
            base_model_name=base_model_name, use_lora=use_lora, lora_r=lora_r,
            lora_alpha=lora_alpha, lora_dropout=lora_dropout, vision_dim=vision_dim,
            q_hidden_size=q_hidden_size, q_nhead=q_nhead, q_enc_layers=q_enc_layers,
            q_dec_layers=q_dec_layers, q_num_query_tokens=q_num_query_tokens
        )
        self.llama_hidden_size = self.mllm.llama_hidden_size
        self.ltsf = TransformerLTSF(
            seq_len=seq_len, out_len=out_len, individual=individual,
            feature_size=feature_size, d_model=d_model,
            polygon_embed_dim=lane_polygon_d_model, use_post_mlp=use_post_mlp,
            post_mlp_hidden_dim=post_mlp_hidden_dim,
            post_mlp_output_dim=d_model*out_len,
    ...
    def forward(self, x, vision_embs, context_str, lane_polygon_batch,
                lane_polygon_len, y=None, norm_stat=None, input_ids=None,
                attention_mask=None, labels=None):
        device = x.device
        B = x.size(0)
        poly_emb = self.lane_polygon_encoder(lane_polygon_batch, lane_polygon_len)
        final_hidden, _ = self.mllm(vision_embs, context_str,
                                    input_ids=input_ids,
                                    attention_mask=attention_mask,
                                    labels=labels)
...
def visualize_one_sample(model, sample, device, idx=0,
                         prefix="test_sample", save_dir="3_5_visualization_test_ddp"):
    ...
    debug_print(f"Visualizing sample {idx}")
    with torch.no_grad():
    ...
    plt.close()
    debug_print(f"Sample {idx} visualization saved to {save_path}")
...
def visualize_test_samples(model, dataset, device, num_samples=5,
    ...
    debug_print(f"Visualizing {len(indices)} test samples")
    for i, idx in enumerate(indices):
        sample = dataset[idx]
        visualize_one_sample(model, sample, device, idx=idx,
                             prefix=prefix, save_dir=save_path)
...
def train_ddp(args):
    ...
    debug_print("Process group initialized", force=True)
    with open(args["all_data_pkl"], 'rb') as f:
        all_data = pickle.load(f)
    debug_print("Loaded all data from pickle", force=True)
    all_data = check_data_sanity(all_data, max_coord_threshold=1e6)
    ...
    debug_print("Data loaders created")
    ...
    debug_print("DDP model wrapped")
    trainable_params = [p for p in ddp_model.parameters() if p.requires_grad]
    optimizer = torch.optim.AdamW(trainable_params, lr=args["lr"], weight_decay=1e-4)
    epochs = args["epochs"]
    best_val_loss = float('inf')
    best_ckpt_path = "3_5_best_val_checkpoint.pt"

    # 다중 모달 예측 방식의 테스트 코드 (후보 예측을 통해 minADE, minFDE, minRMSE 계산)
    if local_rank == 0 and os.path.exists(best_ckpt_path):
        debug_print("====== [TEST] Best checkpoint exists. Starting testing... ======", force=True)
        best_state = torch.load(best_ckpt_path, map_location=device)
        ddp_model.module.load_state_dict(best_state)
        debug_print(f"Checkpoint loaded: {best_ckpt_path}", force=True)
        ...
        debug_print("Test dataset built", force=True)
        test_dataset = MultiModalTrajectoryDataset(test_inputs, test_outputs,
                                                     max_polygon_points=args["max_polygon_points"])
        test_loader = DataLoader(test_dataset, batch_size=args["batch_size"], shuffle=False,
                                 collate_fn=custom_collate_fn)
        debug_print(f"Test DataLoader created, {len(test_loader)} batches", force=True)
        ...
        total_min_rmse = 0.0
        total_samp = 0
        # 후보 예측을 위해 모델을 train 모드로 전환 (gradient 계산 없이)
        ddp_model.eval()
        with torch.no_grad():
            for batch_data in test_loader:
                ...
                for i in range(num_candidates):
                    ...
                    if isinstance(out, tuple):
                        ...
                        pred = out
                    # 모델 출력은 (B, 2, T_out), 전치하여 (B, T_out, 2)
                ...
                total_samp += B_
                debug_print(f"Batch processed: mean min ADE = {min_ade.mean().item():.4f}, "
                            f"mean min FDE = {min_fde.mean().item():.4f}, "
                            f"mean min RMSE = {min_rmse.mean().item():.4f}", force=True)
        ...
        for i in range(min(5, len(test_dataset))):
            ...
            visualize_one_sample(ddp_model.module, sample, device, idx=i,
                                  prefix="test_sample", save_dir="3_5_train_visualization_test")
    else:
        debug_print("No best checkpoint exists. Skipping testing.", force=True)
    dist.barrier()

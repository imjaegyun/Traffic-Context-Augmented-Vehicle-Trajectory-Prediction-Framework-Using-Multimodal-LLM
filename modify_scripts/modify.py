...
def check_data_sanity(all_data, max_coord_threshold=1e6):
    """
    샘플별로 raw_trajectory에 NaN/Inf나 극단적인 값이 있는지 확인하여
    정상 샘플만 반환.
    """
    clean_data = []
    for d in all_data:
...
def is_trajectory_abnormal(raw_traj, lane_label=None,
    ...
    """
    주어진 raw_traj에서 스텝 간 거리(diff)가 너무 큰지,
    속도 변화량이 너무 큰지, 이동 방향이 lane_label과 모순되는지 등을 검사.
    """
    ...
    for track_idx, item in enumerate(track_list):
        ...
        if vision_emb is not None:
            ...
            if isinstance(vision_emb, torch.Tensor):
                ...
                emb_np = vision_emb  # 이미 numpy array일 수도 있음

            if not np.all(np.isfinite(emb_np)):
                # NaN/Inf가 있으면 이 트랙(샘플)을 스킵
                continue
        ...
        for start in range(0, N - (seq_len + out_len) + 1, stride):
            ...
            if vision_emb is not None:
                if isinstance(vision_emb, np.ndarray):
                    ...
                    vision_tensor = vision_emb  # 이미 torch.Tensor 상태
...
class LlamaWithCrossAttnPEFT(nn.Module):
    def __init__(self,
        ...
        if use_lora:
            # target_modules=["q_proj","v_proj"] -> q_proj/v_proj만 LoRA 적용
...
def visualize_one_sample(model, sample, device, idx=0,
                         prefix="test_sample", save_dir="`5_1_`visualization_test_ddp"):
...
def train_ddp(args):
    ...
    # (1) 데이터 로드 및 NaN/Inf 스킵 처리
    with open(args["all_data_pkl"], 'rb') as f:
        all_data = pickle.load(f)

    # 필요시 아래 함수 사용해 데이터 정리
    ...
    # (3) Optimizer 설정
    # 여기서 LLM 파라미터는 이미 requires_grad=False 이므로 제외됨
    trainable_params = [p for p in ddp_model.parameters() if p.requires_grad]
    optimizer = torch.optim.AdamW(trainable_params, lr=args["lr"], weight_decay=1e-4)

    epochs = args["epochs"]
    best_val_loss = float('inf')
    best_ckpt_path = "`5_1_`best_val_checkpoint.pt"
    ...
    for epoch in range(epochs):
        ...
        for step, batch_data in enumerate(train_loader):
            ...
            if torch.isfinite(loss):
                nn_utils.clip_grad_norm_(ddp_model.parameters(), max_norm=1.0)
                optimizer.step()
            else:
                print(f"[Warning][Rank={local_rank}] NaN loss detected at step {step}. Skip update.")
                optimizer.zero_grad()
        ...
        if local_rank == 0:
            ...
            if len(val_dataset) > 0:
                ...
                visualize_one_sample(ddp_model.module, sample_val, device, idx=epoch,
                                     prefix="val_sample", save_dir="`5_1_`train_visualization_val")

    # (5) Best ckpt 로드 후 테스트
    if local_rank == 0 and os.path.exists(best_ckpt_path):
            print("====== [TEST] Best checkpoint 존재. 테스트를 시작합니다. ======")
            best_state = torch.load(best_ckpt_path, map_location=device)
            ddp_model.module.load_state_dict(best_state)
            print(f"[TEST] 체크포인트 로드 완료: {best_ckpt_path}")
            ...
            print(f"[TEST] 테스트 DataLoader 생성 완료, 배치 수: {len(test_loader)}")
            
            ddp_model.eval()
            total_ade = 0.0
            total_fde = 0.0
            total_samp = 0
            print(f"[TEST] 시작: 전체 테스트 배치 수 = {len(test_loader)}")
            
            with torch.no_grad():
                for batch_idx, batch_data in enumerate(test_loader):
                    print(f"[TEST] 배치 {batch_idx+1}/{len(test_loader)} 처리 중...")
                    ...
                    total_samp += B_
                    print(f"[TEST] 배치 {batch_idx+1} : 평균 ADE = {ade_batch.mean().item():.4f}, 평균 FDE = {fde_batch.mean().item():.4f}")

            avg_ade = total_ade / total_samp if total_samp > 0 else 0.0
            avg_fde = total_fde / total_samp if total_samp > 0 else 0.0
            print(f"[TEST 결과] 전체 샘플: {total_samp}, 평균 ADE: {avg_ade:.4f}, 평균 FDE: {avg_fde:.4f}")

            visualize_test_samples(ddp_model.module, test_dataset, device,
                                   num_samples=5, prefix="test_sample", save_dir="`5_1_`test_visualization")

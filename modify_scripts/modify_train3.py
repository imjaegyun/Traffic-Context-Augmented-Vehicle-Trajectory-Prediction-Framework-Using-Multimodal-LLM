...
# Prompt+Answer -> input_ids/labels (텍스트 생성 관련 부분은 그대로 두었으나
# 모델에서는 사용하지 않습니다.)
########################################
def build_dataset_from_tracks_sliding(track_list,
    ...
    for track_idx, item in enumerate(track_list):
        ...
        for start in range(0, N - (seq_len + out_len) + 1, stride):
            ...
            # Prompt+Answer (텍스트 생성 부분 – 모델에서는 사용하지 않음)
            prompt_text = (
...
########################################
# LTSF 관련 모듈 (SelfAttentionBlock, LTSF_NLinearEncoder/Decoder, TransformerLTSF)
########################################
...
class MultiModalTrajectoryModel(nn.Module):
    def __init__(self,
        ...
        # MLLM 관련 모듈은 제거하고, 대신 LlamaWithCrossAttnPEFT를 직접 사용
        ...
        # tokenizer 직접 초기화 (텍스트 생성은 사용하지 않음)
    ...
    def forward(self,
                x,                # (B,2,T_in)
                vision_embs,      # (B,T_in,D) – (미사용 가능, 단지 구조상 전달)
                lane_polygon_batch,
                lane_polygon_len,
                y=None,
                norm_stat=None):
        ...
        # Llama 모델을 이용하여 더미 텍스트의 hidden state를 얻음
        dummy_text = "dummy"
        dummy_inputs = self.tokenizer(dummy_text, return_tensors="pt", padding=True, truncation=True).to(device)
        text_embeds = self.llama_wrapper.llama_model.get_input_embeddings()(dummy_inputs["input_ids"])
        ...
        final_hidden = outputs.hidden_states[-1]  # (1, L_text, hidden)
        final_hidden = final_hidden.expand(B, -1, -1)
...
def visualize_one_sample(model, sample, device, idx=0,
                         prefix="test_sample", save_dir="1_1_visualization_test_ddp"):
    ...
    with torch.no_grad():
        out = model(x_in, vision, polygon, poly_len,
...
def train_ddp(args):
    ...
    # MLLM 체크포인트 로딩 부분과 텍스트 생성 관련 코드는 제거되었습니다.
    ...
    best_ckpt_path = "1_1_modify_best_val_checkpoint.pt"
    ...
    for epoch in range(epochs):
        ...
        if local_rank == 0:
            ...
            visualize_one_sample(ddp_model.module, sample_val, device, idx=epoch,
                                 prefix="val_sample", save_dir="1_1_modify_train_visualization_val")
    ...
    if local_rank == 0:
        ...
        with torch.no_grad():
            ...
            print(f"[Test] minADE={avg_min_ade:.4f}, minFDE={avg_min_fde:.4f}, minRMSE={avg_min_rmse:.4f} (Model)")
            ...
            for i in range(min(5, len(test_dataset))):
                ...
                visualize_one_sample(ddp_model.module, sample, device, idx=i,
                                    prefix="test_sample", save_dir="1_1_modify_test_visualization")
...
def main():
    args = {
        ...
        "image_height": 2160,
        "max_polygon_points": 64,
        # MLLM 관련 키는 제거 (대신 base_model_name으로 Llama‑3.2‑1B 사용)
        "base_model_name": "meta-llama/Llama-3.2-1B",
        "use_lora": True,
        "lora_r": 8,
        "lora_alpha": 32,

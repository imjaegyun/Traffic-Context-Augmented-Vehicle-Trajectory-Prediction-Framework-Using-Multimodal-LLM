...
class LlamaWithCrossAttnPEFT(nn.Module):
    def __init__(self,
        ...
        if use_lora:
            ...
            # 이제 LoRA가 붙은 부분("lora_"가 들어간 파라미터)만 학습
            # 나머지는 모두 동결
...
def visualize_one_sample(model, sample, device, idx=0,
                         prefix="test_sample", save_dir="3_3_visualization_test_ddp"):
...
def train_ddp(args):
    ...
    best_ckpt_path = "3_3_best_val_checkpoint.pt"
        ...
        if local_rank == 0:
            ...
            if len(val_dataset) > 0:
                ...
                visualize_one_sample(ddp_model.module, sample_val, device, idx=epoch,
                                     prefix="val_sample", save_dir="3_3_train_visualization_val")
    ...
    if local_rank == 0 and os.path.exists(best_ckpt_path):
            ...
            visualize_test_samples(ddp_model.module, test_dataset, device,
                                num_samples=5, prefix="test_sample", save_dir="3_3_test_visualization")
